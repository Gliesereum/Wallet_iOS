...
class WorkerAddCommentsDialog: UIViewController, NVActivityIndicatorViewable, FloatRatingViewDelegate{
    
    var ratingText: Int = 12
    let constants = Constants()
    var delegate: SingleWorker?
    let utils = Utils()
...
//        let carInfo: CarWashBody?
    ...
    func addComments(){
        let carWashId: String = (delegate?.worker?.id)!
        startAnimating()
        let restUrl = constants.startUrl + "karma/v1/worker/\(carWashId)/comment"
